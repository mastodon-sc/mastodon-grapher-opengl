		...
		OffsetAxesListener
{
	 ...
	 */
	private static final double MIN_WIDTH = 0.2;

	/**
	 * Sets the maximal zoom level in Y.
	 */
	private static final double MIN_HEIGHT = 0.2;
	...
	public synchronized void layoutChanged( final float[] xy )
	{
		...
		boundXMin = minX - boundXLayoutBorder;
		boundXMax = maxX + boundXLayoutBorder;
		boundYMin = minY - boundYLayoutBorder;
		boundYMax = maxY + boundYLayoutBorder;

		final ScreenTransform transform = transformState.get();
		if ( boundXMax - boundXMin < MIN_WIDTH )
		{
			final double c = ( boundXMax + boundXMin ) / 2;
			boundXMin = c - MIN_WIDTH / 2;
			boundXMax = c + MIN_WIDTH / 2;
		}
		updateMaxSizeX( transform.getScreenWidth() );
		if ( boundYMax - boundYMin < MIN_WIDTH )
		{
			final double c = ( boundYMax + boundYMin ) / 2;
			boundYMin = c - MIN_WIDTH / 2;
			boundYMax = c + MIN_WIDTH / 2;
		}
		updateMaxSizeY( transform.getScreenHeight() );
	...
	public void setLayoutRangeY( final double layoutMinY, final double layoutMaxY )
	{
		...
		if ( boundYMax - boundYMin < MIN_HEIGHT )
		{
			final double c = ( boundYMax + boundYMin ) / 2;
			boundYMin = c - MIN_HEIGHT / 2;
			boundYMax = c + MIN_HEIGHT / 2;
		}
	...
	{
		return ConstrainScreenTransform.hasMinSizeX( transform, MIN_WIDTH + EPSILON );
	}

	private boolean hasMinSizeY( final ScreenTransform transform )
	{
		return ConstrainScreenTransform.hasMinSizeY( transform, MIN_HEIGHT + EPSILON );
	}
